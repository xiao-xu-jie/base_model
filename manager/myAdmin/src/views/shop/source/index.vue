...
import {
  addbizSourceStation,
  deletebizSourceStation,
  getbizSourceStationList,
  updatebizSourceStation
} from "@/api/source";
...
async function onbatchDel(id = null) {
  ...
  const res = await deletebizSourceStation(ids);
  if (res.success) {
  ...
  addDialog({
    ...
    props: {
      formInline: {
        ...
        uid: item?.uid ?? "",
        secret: item?.secret ?? "",
        stationStatus: item?.stationStatus ?? 1
      }
    },
    ...
    beforeSure: async (done, { options, closeLoading }) => {
      ...
      try {
        res =
          title === "编辑"
            ? await updatebizSourceStation(options.props.formInline)
            : await addbizSourceStation(options.props.formInline);
      } catch (e) {
        console.log("e===>>>: ", e);
      } finally {
        closeLoading();
...
const loadData = async (flag = 1) => {
  ...
  let res;
  try {
    res = await getbizSourceStationList({
      ...searchForm.value,
  ...
  <div>
    <div class="header">
      <el-form
        ...
        :model="searchForm"
      >
        <el-form-item label="货源名" prop="stationName">
          <el-input
    ...
    <PureTableBar :columns="columns" title="货源管理" @refresh="loadData">
      <template #buttons>
        <el-button
          type="primary"
      ...
      <template v-slot="{ size, dynamicColumns }">
        ...
        >
          <template #secret="{ row }">
            <el-tag size="small">{{ row.secret }}</el-tag>
          </template>
          <template #stationStatus="{ row }">
            <el-tag
              :type="row.stationStatus === 1 ? 'success' : 'danger'"
              size="small"
            >
              {{ row.stationStatus === 1 ? "启用" : "禁用" }}
            </el-tag>
          </template>
