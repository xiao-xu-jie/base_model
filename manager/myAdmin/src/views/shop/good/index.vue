...
async function onbatchDel(id = null) {
  ...
  const res = await deletebizGood(ids);
  if (res.success) {
  ...
  addDialog({
    ...
    beforeSure: async (done, { options, closeLoading }) => {
      ...
      try {
        res =
          title === "编辑"
            ? await updatebizGood(options.props.formInline)
            : await addbizGood(options.props.formInline);
      } catch (e) {
        console.log("e===>>>: ", e);
      } finally {
        closeLoading();
...
<template>
  <div>
    ...
    <PureTableBar :columns="columns" title="商品管理" @refresh="loadData">
      <template #buttons>
        <el-button
          type="primary"
