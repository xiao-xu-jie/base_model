...
async function onbatchDel(id = null) {
  ...
  const res = await deleteSlider(ids);
  if (res.success) {
  ...
  addDialog({
    ...
    beforeSure: async (done, { options, closeLoading }) => {
      ...
      try {
        res =
          title === "编辑"
            ? await updateSlider(options.props.formInline)
            : await addSlider(options.props.formInline);
      } catch (e) {
        console.log("e===>>>: ", e);
      } finally {
        closeLoading();
  ...
  <div>
    <div class="header">
      <el-form
        ...
        :model="searchForm"
      >
        <el-form-item label="轮播内容" prop="content">
          <el-input
    ...
    <PureTableBar :columns="columns" title="轮播管理" @refresh="loadData">
      <template #buttons>
        <el-button
          type="primary"
      ...
      <template v-slot="{ size, dynamicColumns }">
        ...
        >
          ...
          <template #showStatus="{ row }">
            <el-tag :type="row.showStatus == 1 ? 'success' : 'danger'">
